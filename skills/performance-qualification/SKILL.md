...
|---|---|---|
| R version, numerical backend **and** all critical packages match | **Strict** | `expect_equal()`, bit-for-bit |
| R version, backend **or** any critical package differs | **Tolerant** | `max(abs(object - reference)) <= tolerance` |
